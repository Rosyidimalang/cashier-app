...
export default function CobaAxios() {
  ...
  const save = (e) => {
    ...
    //   gambar: "test",
    // };
    axios.post("http://localhost:4000/makanan", data).then(() => ambilData());
  ...
  const del = (id, e) => {
    ...
    axios.delete("http://localhost:4000/makanan" + id).then(() => ambilData());
  };
  ...
  const edit = (e, id) => {
    ...
    axios
      .patch("http://localhost:4000/makanan/" + id, data)
      // ganti sebagian
      .put("http://localhost:4000/makanan/" + id, data)
      // ganti totalitas
      .then(() => ambilData());
    setisEdit(null);
  };
    ...
    <div>
      <form onSubmit={save}>
        ...
        <input
          ...
          className="border border-red-800"
        ></input>
        <input name="gambar" type={"file"}></input>
